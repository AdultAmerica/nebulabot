...
HELP = {
    ...
    "buttons": (
        ...
        "<b>Getting buttons and media into one post.</b> Telegram accepts a "
        "keyboard on a single photo or video, and refuses one on an album — "
        "there is no setting that changes this. So:\n\n"
        "• <b>One item in the group</b> — caption and buttons ride on it. One "
        "self-contained post.\n"
        "• <b>Several items, 📮 One item per post on</b> — each run sends one "
        "of them with the caption and buttons attached, taking turns. Still "
        "one post every time, and everything gets shown in rotation. This is "
        "the setting you want for promos.\n"
        "• <b>Several items, sent as an album</b> — two messages, always. The "
        "🔘 <b>Buttons</b> option only chooses whether the keyboard sits below "
        "the album or on a first item split off from it.\n\n"
        "<code>/health</code> shows whether this bot's Telegram allows album "
        "keyboards at all."
    ),
    "schedule": (
        "⏱ <b>Scheduling</b>\n\n"
